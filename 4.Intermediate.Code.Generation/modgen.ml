...
and make_int_simplexpr (depth:int) (st:symtab) : simplexpr = 
  ...
  | 1 -> let op = (match Random.int 2 with 0 -> Pos | _ -> Neg) in
          Simplexpr1 (op, (make_int_term (depth-1) st))
  | _ -> let op = (match Random.int 2 with 0 -> Plus | _ -> Minus) in
...
and make_int_term (depth:int) (st:symtab) : term =
  ...
  | _ -> let op = (match Random.int 2 with 0 -> Mult | _ -> Div) in
...
and make_bool_expr (depth:int) (st:symtab) : expr =
  ...
  | 1 -> let op = (match Random.int 2 with 0 -> Eq | _ -> Neq) in
         let f = (match Random.int 3 with 0 -> make_int_simplexpr | 1 -> make_bool_simplexpr | _ -> make_char_simplexpr) in
         Expr1 ((f (depth-1) st), op, (f (depth-1) st))
  | _ -> let op = (match Random.int 4 with 0 -> Lt | 1 -> Le | 2 -> Gt | _ -> Ge) in
         let f = (match Random.int 2 with 0 -> make_int_simplexpr | _ -> make_char_simplexpr) in
         Expr1 ((f (depth-1) st), op, (f (depth-1) st))
...
and make_bool_factor (depth:int) (st:symtab) : factor =
  ...
  | 1 -> let v = (match Random.int 2 with 0 -> true | _ -> false) in
         Factor2 v
